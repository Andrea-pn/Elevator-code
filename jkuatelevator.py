...
def select_lift(floor, current_elevator=None):
    # If we're already in an elevator going to ground floor, use that elevator
    if current_elevator and floor == 0:
...
#def go_to_floor(floor):
     ...
     #       threading.Thread(target=elevator.process_request, args=(floor, status_label)).start()
    #else:
...
# Create a variable to track which elevator is "active" (simulating being inside)
active_elevator = tk.StringVar(value="None")

def set_active_elevator(elevator_label):
    active_elevator.set(elevator_label)
    status_label.config(text=f"Now inside Elevator {elevator_label}", fg=button_colors[elevator_label])

# Create elevator selector buttons (to simulate being inside a specific elevator)
elevator_selector_frame = tk.Frame(control_frame, bg="#ffffff", pady=10)
elevator_selector_frame.pack()

tk.Label(elevator_selector_frame, text="Simulate being inside elevator:", 
         bg="#ffffff", fg="#555555").pack(side=tk.LEFT, padx=5)

for label in ["A", "B", "C"]:
    btn = tk.Button(
        elevator_selector_frame,
        text=label,
        width=3,
        bg=button_colors[label],
        fg="white",
        font=("Arial", 10, "bold"),
        relief=tk.FLAT,
        cursor="hand2",
        command=lambda l=label: set_active_elevator(l)
    )
...
tk.Button(
    ...
    relief=tk.FLAT,
    cursor="hand2",
    command=lambda: active_elevator.set("None")
).pack(side=tk.LEFT, padx=5)

# Function to handle floor button presses
def handle_floor_button(floor):
    current_elev = active_elevator.get()
    
    # If we're inside an elevator
    if current_elev != "None":
        elevator = None
        if current_elev == "A":
            elevator = elevator_A
        elif current_elev == "B":
            # Allow B to go to ground floor (0) or its assigned floors (6-8)
            if floor == 0 or (6 <= floor <= 8):
                elevator = elevator_B
            else:
                status_label.config(text=f"Elevator B can only access floors 0, 6-8", fg="red")
                return
        elif current_elev == "C":
            # Allow C to go to ground floor (0) or its assigned floors (9-10)
            if floor == 0 or (9 <= floor <= 10):
                elevator = elevator_C
            else:
                status_label.config(text=f"Elevator C can only access floors 0, 9-10", fg="red")
                return
...
for i in range(10, -1, -1):
    ...
    btn = tk.Button(
        ...
        relief=tk.FLAT,
        cursor="hand2",
        command=lambda f=i: handle_floor_button(f)
    )
...
# Replace the floor label creation code with this:
# First, let's identify what's causing the doubling and fix it
# This code replaces the entire floor drawing section
...
for i in range(num_floors):
    ...
    # Create floor labels with consistent background
    label_width = 65  # Increased width for all labels
    label_height = 24  # Fixed height for all labels
    
    # Improved label background with consistent sizing
    label_bg = canvas.create_rectangle(
        left_margin - label_width - 5, floor_y + floor_height//2 - label_height//2, 
        left_margin - 5, floor_y + floor_height//2 + label_height//2,
        fill="#e0e0e0", outline="#a0a0a0", width=1
    )
    
    # Improved floor label text with better positioning - this is the ONLY floor label we want
    canvas.create_text(
        left_margin - label_width//2 - 5, floor_y + floor_height//2,
        text=f"FLOOR {i}",
        fill="#333333",
        font=("Arial", 10, "bold"),
    ...
    # REMOVE THIS TEXT CREATION - It's causing the doubling
    # Instead, we can add a descriptive text if needed
    #canvas.create_text(
       # left_margin + 80, floor_y + floor_height//2,
        #text=f"Department {i}",  # Changed to descriptive text instead of duplicating the floor number
        #fill="#333333",
        #font=("Arial", 9, "bold"),
        #anchor="center"
    #)
